...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  // setup and run EdgeBoxGenerator
    ...
    pl[0] = mxCreateNumericMatrix(num_box, 5, mxSINGLE_CLASS, mxREAL);
    float *bbs = (float *)mxGetData(pl[0]);

    for (int i = 0; i < num_box; i++)
    {
        std::cout<<bboxes_f[i]<<" "<<bboxes_f[i+num_box]<<" "<<bboxes_f[i+num_box*2]<<" "<<bboxes_f[i+num_box*3]<< " "<<num_box<<std::endl;
        Box b;
