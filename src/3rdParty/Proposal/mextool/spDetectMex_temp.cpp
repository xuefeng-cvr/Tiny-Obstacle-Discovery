...
void sticky(uint *S, uint *S_iter, uint h, uint w, float *I, float *E, double *prm)
{
    // get additional parameters
    const uint maxIter = uint(prm[0]);
    uint nThreads = uint(prm[1]);
...
#pragma omp parallel for num_threads(nThreads)
        ...
        memcpy(S_iter + iter * h * w, S, h * w * sizeof(uint));
        relabel(S_iter + iter * h * w, h, w);
        iter++;
    }
    delete[] ns;
    delete[] mus;
...
#pragma omp parallel for num_threads(nThreads)
        ...
        for (int xi = 0; xi < int(w); xi++)
            for (uint y = 0; y < h; y++)
            {
                uint x = xi, a = x * h + y, b = (x + 1) * h + y, c = x * h + (y + 1), s = S[a];
                if (s == 0)
                    continue;
                ...
                uint8 *segs, uint *dims, uint nThreads)
{
    ...
    float *wts = new float[w * h];  // ȫͼÿһ�����ص��Ȩֵ
    float *Sn = new float[m * m](); // �����Ĺ�ϵ����
    float *Sd = new float[m * m](); // �����Ĺ�ϵ����

    for (uint i = 0; i < w * h; i++)
        wts[i] = 1 / (1 + exp((E[i] - .05f) * 50.f)); // ÿ�����ص�ĳ�ʼȨֵ
...
#pragma omp parallel for num_threads(nThreads)
    ...
    // �������е�patch
    for (int x = 0; x < int(w); x += stride) //������һ��
    {
        ...
        lbls1 = new uint[g * g];
        wts1 = new float[m * 25];
        for (y = 0; y < int(h); y += stride) //������һ��
        {
            // ÿ��patch����һ��
            // create consecutively labeled copy of local label mask �������ɾֲ���ǩ����ı�ǩ����
            x0 = max(-r, -x);
            x1 = min(r, int(w) - x);
            y0 = max(-r, -y);
            y1 = min(r, int(h) - y);              // patch��������ڵ�ǰ���ص�λ��
            lookup[0] = S[(x + x0) * h + y + y0]; // ��patch�е�һ�����ص�superpixel��labelֵ
            wts1[0] = 0;
            j = 0;
            m1 = 1; // ��ǰ������patch�г��ֵ���������
            nTreesConst = 0;
            ...
            for (xi = x0; xi < x1; xi++)
                for (yi = y0; yi < y1; yi++)
                {
                    ...
                    lbls1[(xi + r) * g + yi + r] = i; // ��¼label
                    j = i;
                    wts1[i] += wts[(x + xi) * h + y + yi]; // ͳ��Ȩֵ
                }

            // loop over nTreesEval segmentation masks ѭ��nTreesEval�ηָ�����
            for (t = 0; t < nTreesEval; t++)
            {
                ind = y / stride + x / stride * h1 + t * h1 * w1; // y��x��patch�ڵ�t�����ϵ�����
                const uint8 *seg = segs + ind * g * g;            // ͨ��ind�ҵ�seg
                s1 = 0;                                           // ��ǰ������patch(seg)�е�superpixel������
                // compute number of segments s1 in seg  ����seg�е�s1�ֶε�����
                for (xi = x0; xi < x1; xi++) // �ٴα���patch������
                    for (yi = y0; yi < y1; yi++)
                    {
                        i = (xi + r) * g + yi + r; // patch��ÿ�����ص�����
                        if (seg[i] > s1)
                            s1 = seg[i]; // ��seg��������
                    }
                s1++;
                if (s1 == 1) // ���patch(seg)����ֻ��һ�����ص�
                {
                    nTreesConst++;
                    continue;
                }
                ...
                for (xi = x0; xi < x1; xi++)
                    ...
                    {
                        i = (xi + r) * g + yi + r;
                        wts1[lbls1[i] + (seg[i] + 1) * m1] += wts[(x + xi) * h + y + yi]; // ��¼Ȩֵ
                    }
                ...
                for (s = 1; s <= s1; s++)
                    for (i = 0; i < m1; i++)
                        for (j = 0; j < m1; j++)
                            Sn[lookup[i] * m + lookup[j]] += wts1[i + s * m1] * wts1[j + s * m1]; // �ۼӵ�i���͵�j��Ȩֵ�˻�
            }
            // update Sn same way as Sd for skipped uniform patches
            // Ϊ������ͳһpatches����sdһ���ķ�������sn
            if (nTreesConst)
                for (i = 0; i < m1; i++)
                    for (j = 0; j < m1; j++)
...
void edges(float *E, uint *S, uint h, uint w, float *A)
{
    ...
    for (x = 0; x < w; x++)
        for (y = 0; y < h; y++)
        {
            ...
            y1 = (y == h - 1) ? h - 1 : y + 1;
            // ������ǰ���ص�8����
            for (xi = x0; xi <= x1; xi++) // ͳ�ư�����������Щsp���򣬱�����ss�����У�n��������
                for (yi = y0; yi <= y1; yi++)
                {
                    s = S[xi * h + yi];
                    if (!s) // ������Ե����
...
void mexFunction(int nl, mxArray *pl[], int nr, const mxArray *pr[])
{
    ...
    else if (!strcmp(action, "sticky"))
    {
        ...
        pl[1] = mxCreateNumericArray(3, dims, mxUINT32_CLASS, mxREAL);
        uint *T = (uint *)mxGetData(pl[0]);
        uint *S_iter = (uint *)mxGetData(pl[1]);
        memcpy(T, S, h * w * sizeof(uint));
        sticky(T, S_iter, h, w, I, E, prm);
        relabel(T, h, w);
        // mxSetDimensions(pl[1], (const mwSize *) datasize,8);
    }
    ...
    else if (!strcmp(action, "affinities"))
    {
        ...
        uint nThreads = (uint)mxGetScalar(pr[3]);
        if (mxGetNumberOfDimensions(pr[2]) != 5)
