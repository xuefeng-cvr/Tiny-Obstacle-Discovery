...
typedef struct
{
	int c, r, w, h;
	float s;
} Box;  //location(x,y),width,height and score
...
public:
	...
	int _maxBoxes;
	float _maxAspectRatio, _minBoxArea, _kappa;
...
void EdgeBoxGenerator::clusterEdges(arrayf &E, arrayf &O, arrayf &V)
{
	...
	_segIds.init(h, w);  //edge segment index
	_segCnt = 1;  //edge segment count

	for (c = 0; c<w; c++)
		for (r = 0; r<h; r++)
		{
			if (c == 0 || r == 0 || c == w - 1 || r == h - 1 || E.val(c, r) <= 0.1)
				_segIds.val(c, r) = -1;
			else _segIds.val(c, r) = 0;
		}
	for (c = 1; c<w - 1; c++)  //edge merge
		for (r = 1; r<h - 1; r++)
		{
	...
	for (c = 1; c<w - 1; c++) for (r = 1; r<h - 1; r++)
		if ((j = _segIds.val(c, r))>0 && _segMag[j] <= 0.5)
			_segIds.val(c, r) = 0;
	i = 1;
	while (i>0)
	{
...
void EdgeBoxGenerator::prepDataStructs(arrayf &E)
{
	...
	_vIdxs.resize(w); //vertical boundary
	_vIdxImg.init(h, w);
	for (c = 0; c<w; c++)
	{
		int s = 0;
...
void EdgeBoxGenerator::scoreBox(Box &box)
{
	...
	for (i = 0; i<n; i++)
	{
		float w = sWts[i];  //weight
		j = sIds[i];
		for (k = 0; k<int(_segAffIdx[j].size()); k++) 
		{
			q = _segAffIdx[j][k]; // adjacency
			float wq = w*_segAff[j][k]; 
			if (wq<.05f) continue; // short circuit for efficiency
			if (sDone[q] == sId) 
			{
				if (wq>sWts[sMap[q]]) //update the route
                {
					sWts[sMap[q]] = wq; //affinity
					i = min(i, sMap[q] - 1); //current index
				}
			}
	...
	for (i = 0; i<n; i++) // traverse all overlapped edge seg
	{
		k = sIds[i];  //edge index
...
void EdgeBoxGenerator::refineBox(Box &box) //scale finetune
{
	int rStep = int(box.h*_rcStepRatio); //row step
	int cStep = int(box.w*_rcStepRatio); //column step
	while (1)  
	{
		...
		// down
		B = box;
		B.h = B.h + rStep; //increase height
		scoreBox(B);
		if (B.s <= box.s)
		{
			B = box;
			B.h = B.h - rStep; //decrease height
			scoreBox(B);
		}
		if (B.s>box.s) box = B;  
		//left
		B = box;
		B.c = box.c - cStep; //column 
		B.w = B.w + cStep; //width
		scoreBox(B);  
		if (B.s <= box.s)
		{
			B = box;
			B.c = box.c + cStep; //column increases a step
			B.w = B.w - cStep; //width decreases a step
			scoreBox(B); 
		}
		if (B.s>box.s) box = B; //update box
		//right width
		B = box;
		B.w = B.w + cStep;
		scoreBox(B);
...
void EdgeBoxGenerator::drawBox(Box &box, arrayf &E, arrayf &V)
{
	...
	r = r0;
	for (c = c0; c <= c1; c++) V.val(c + w * 0, r) = V.val(c + w * 1, r) = V.val(c + w * 2, r) = 0;
	r = r1;
	for (c = c0; c <= c1; c++) V.val(c + w * 0, r) = V.val(c + w * 1, r) = V.val(c + w * 2, r) = 0;
	c = c0;
	for (r = r0; r <= r1; r++) V.val(c + w * 0, r) = V.val(c + w * 1, r) = V.val(c + w * 2, r) = 0;
	c = c1;
	for (r = r0; r <= r1; r++) V.val(c + w * 0, r) = V.val(c + w * 1, r) = V.val(c + w * 2, r) = 0;
}
...
void mexFunction(int nl, mxArray *pl[], int nr, const mxArray *pr[])
{
	...
	edgeBoxGen._beta = float(mxGetScalar(pr[2]) + tol);
    edgeBoxGen._kappa = float(mxGetScalar(pr[3]));
	edgeBoxGen._minScore = float(mxGetScalar(pr[4]));
	edgeBoxGen._maxBoxes = int(mxGetScalar(pr[5]));
	edgeBoxGen._maxAspectRatio = float(mxGetScalar(pr[6]));
	edgeBoxGen._minBoxArea = float(mxGetScalar(pr[7]));
