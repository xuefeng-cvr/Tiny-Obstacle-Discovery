...
void EdgeBoxGenerator::prepDataStructs( arrayf &E )
{
    ...
    for( c=1; c<w; c++ ) for( r=1; r<h; r++ ) //edge groupå¼ºåº¦ç§?????
        {
...
void EdgeBoxGenerator::scoreBox( Box &box )
{
    ...
    for( i=cs; i<=ce; i++ ) if( (j=_hIdxs[r0][i])>0 && sDone[j]!=sId ) //ç´¢å?>0ï¼???edge group
        {
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
    ...
    box.r= int(mxGetScalar(pr[14]));  //è¡?
    box.w= int(mxGetScalar(pr[15])); //å®?
    box.h= int(mxGetScalar(pr[16]));  //é«?
